...
contract Upgrader is Initializable{
	...
	function initializeContract(
		...
		address payable _voteMachineAddress,		
		address payable _DAOAddress,	
		address payable _rewardsMachineAddress,	
		address payable _marketFactoryAddress,	
    ...
    */
    function initiateUpgradeVote(
		...
		require (voteNumber > 500000*(10**18),'INSUFFICIENT_veISS');
    	...
    	getUpgradeVotes[_newImplementationAddress].endingTime = block.timestamp.add(7 days);
    ...
    */
    function voteUpgradeVote (
		...
		if (_vote == true) {
			getUpgradeVotes[_newImplementationAddress].yesVotes = getUpgradeVotes[_newImplementationAddress].yesVotes.add(voteNumber);
			
		}
		else {
			getUpgradeVotes[_newImplementationAddress].noVotes = getUpgradeVotes[_newImplementationAddress].noVotes.add(voteNumber);
		}
		
		getUpgradeVotes[_newImplementationAddress].voteNumber = getUpgradeVotes[_newImplementationAddress].voteNumber.add(1);
    ...
    */
    function closeUpgradeVote (
		...
		if (getUpgradeVotes[_newImplementationAddress].yesVotes > getUpgradeVotes[_newImplementationAddress].noVotes){
			...
			ProxyAdmin(proxyAdminAddress).upgrade(proxy, _newImplementationAddress);
